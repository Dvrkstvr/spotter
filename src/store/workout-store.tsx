...
/**
 * What a stored blob becomes is `@/data/migrate`'s answer, not this file's —
 * the durable key list, the shape each must arrive in, the version chain and
 * the backup merge all live there, pure and testable. What stays here is the
 * I/O: the addresses below, and when to call any of it.
 *
 * Three names are re-exported because a screen asks the *store* for them —
 * Settings stamps a backup with `STORAGE_VERSION`, and the restore sheet is
 * built out of `RESTORE_PARTS`.
 */
export { RESTORE_PARTS, STORAGE_VERSION } from '@/data/migrate';
export type { RestoreCounts, RestorePart } from '@/data/migrate';
